...
@Component({
  selector: 'app-regular-panel',
  standalone: true,
  imports: [DatePipe, MatExpansionModule,MatDividerModule,MatProgressBarModule],
  templateUrl: './regular-panel.component.html',
  styleUrl: './regular-panel.component.scss'
})
export class RegularPanelComponent {
  ...
  getValueForDay(week: any, day: string): string {
    ...
    if (dayData && dayData.jodi !== undefined && dayData.jodi !== '') {
      return `<span class="small">${dayData?.open}</span><span>${dayData?.jodi}</span><span class="small">${dayData?.close}</span>`;
    } else {
      return '-';
    }
  }
